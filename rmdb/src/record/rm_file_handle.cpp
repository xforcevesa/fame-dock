...
std::unique_ptr<RmRecord> RmFileHandle::get_record(const Rid &rid,
  ...
  // 初始化一个指针
  return std::make_unique<RmRecord>(file_hdr_.record_size,
...
Rid RmFileHandle::insert_record(char *buf, Context *context) {
  ...
  auto accessible_slot_no =
      Bitmap::first_bit(0, page_handle.bitmap, file_hdr_.num_records_per_page);
  auto accessible_slot = page_handle.get_slot(accessible_slot_no);
  // 将buf复制到空闲的slot位置处
  memcpy(accessible_slot, buf, file_hdr_.record_size);
  Bitmap::set(page_handle.bitmap, accessible_slot_no);
  // 插入了一条新的record后，page_handle中num_records增多
  page_handle.page_hdr->num_records++;
  if (page_handle.page_hdr->num_records ==
      file_hdr_.num_records_per_page) { // 达到page存储的上限
    file_hdr_.first_free_page_no = page_handle.page_hdr->next_free_page_no;
  }
  auto page_id = page_handle.page->get_page_id();
  buffer_pool_manager_->unpin_page(page_id, true);
  return Rid{page_id.page_no, accessible_slot_no};
}
...
void RmFileHandle::delete_record(const Rid &rid, Context *context) {
  ...
  // 相当于insert_record的反向操作
  auto page_handle = fetch_page_handle(rid.page_no);
  if (page_handle.page_hdr->num_records == file_hdr_.num_records_per_page) {
    release_page_handle(page_handle);
  }
  // 由于被释放了recotd,record总数-1
  page_handle.page_hdr->num_records--;
...
RmPageHandle RmFileHandle::create_new_page_handle() {
  ...
  auto new_page_handle = RmPageHandle{&file_hdr_, page};
